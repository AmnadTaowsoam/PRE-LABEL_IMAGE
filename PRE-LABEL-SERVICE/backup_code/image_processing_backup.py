...
def predict_image(image_data):
    """
    Predict image using YOLO models for detection and classification.
    Parameters:
    - image_data: bytes, image data to be processed
    Returns:
    - output_data: list, containing detected and classified objects
    """
    try:
        # Image decoding
            ...
            try:
                ...
                if cls_result:
                    ...
                    output_data.append({
                        ...
                        "value": {
                            "x": x1 / image_width * 100,  # Convert to percentage
                            "y": y1 / image_height * 100,  # Convert to percentage
                            "width": (x2 - x1) / image_width * 100,  # Convert to percentage
                            "height": (y2 - y1) / image_height * 100,  # Convert to percentage
                            "rotation": 0,
                            "rectanglelabels": [max_conf_class]
                        },
                        "score": max_conf
                    })
                else:
                    logger.warning(f"No classification result for box {i+1}")

            except Exception as e:
                logger.error(f"Error in classification for box {i+1}: {e}")

        logger.info(f"Image processed successfully. {len(output_data)} boxes detected and classified.")
        return output_data
